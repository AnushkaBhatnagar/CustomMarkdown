...
if [ ! -f "input.txt" ]; then
    log_message "Error: input.txt not found. Exiting."
    exit 1
fi
...
if [ $LEXER_STATUS -ne 0 ]; then # if status!=0
    ...
    # Parsing Phase
    log_message "Starting Parsing Phase..." #READ FROM HERE
    if [ ! -f "output.txt" ]; then
        log_message "Error: output.txt not found. Exiting."
        #exit 1
    fi
    ...
    if [ $PARSER_STATUS -ne 0 ]; then # if status!=0
        ...
        if [ ! -f "parser_output.txt" ]; then
            log_message "Error: parser_output.txt not found. Exiting."
            exit 1
        fi
